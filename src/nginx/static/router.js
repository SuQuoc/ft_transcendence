...
const getDisplayname = async () => {
	if (window.app.userData.username)
...
const Router = {
	...
	makeWebSocket: (type) => {
		...
		if (type === 'onFindOpponentPage') channel_name = 'match';

		if (!window.app.socket) {
			let ws_scheme = window.location.protocol == 'https:' ? 'wss' : 'ws';
			let ws_path =
				ws_scheme + '://' + window.location.host + '/daphne/pong/' + channel_name + '/';
		...
		window.app.socket.onopen = () => {
			window.app.socket.send(
				JSON.stringify({ type: type, user_id: window.app.userData.username })
			);
		};
	},
	...
	handleSocketMessageChangeRoute(event) {
		...
		switch (data.type) {
			case 'joinTournament':
				...
				else Router.go('/tournament', false); // should probably be false ??
				break;
			default:
