...
export class JoinTournamentPage extends ComponentBaseClass {
    constructor() {
        ...
        this.handleTournamentCreationVar =
            this.handleTournamentCreation.bind(this)
    }
    ...
    connectedCallback() {
        ...
        this.create_tournament_form = this.root.getElementById(
            'createTournamentForm'
        )
        this.range_display = this.root.getElementById(
            'createPointsToWinDisplay'
        )
        ...
        this.create_tournament_form.addEventListener(
            'submit',
            this.handleTournamentCreationVar
        )
    ...
    disconnectedCallback() {
        ...
        this.create_tournament_form.removeEventListener(
            'submit',
            this.handleTournamentCreationVar
        )
        this.input_range.removeEventListener(
            'input',
            this.handleRangeDisplayVar
        )
    }
    ...
    createJoinTournamentElement(
        ...
        element.querySelectorAll("[name='join_name']")[0].innerHTML =
            tournament_name
        element.querySelectorAll("[name='join_creator']")[0].innerHTML =
            creator_name
        element.querySelectorAll("[name='join_points_to_win']")[0].innerHTML =
            points_to_win
        element.querySelectorAll(
            "[name='join_current_player_num']"
        )[0].innerHTML = current_player_num
        element.querySelectorAll("[name='join_max_player_num']")[0].innerHTML =
            max_player_num
    ...
    noTournamentsToJoin() {
        // not working!!!!!
        const tournament_elements = this.root.querySelectorAll(
            'join-tournament-element'
        )
    ...
    handleRangeDisplay(event) {
        ...
        const thumb_position =
            ((event.target.value - min) / (max - min)) * range_width
        const display_position =
            thumb_position - this.range_display.offsetWidth / 2
